...
final quoteDataProvider = FutureProvider.family<Map<String, dynamic>, String>((ref, theme) async {
  final locale = ref.watch(localeProvider);
  final langCode = locale.languageCode;
  final themeLower = theme.toLowerCase();
  
  try {
    final String jsonString = await rootBundle.loadString('assets/data/quotes/$langCode/$themeLower.json');
    return json.decode(jsonString) as Map<String, dynamic>;
  } catch (e) {
    // Robust fallback to English for the same theme
    try {
      final String jsonString = await rootBundle.loadString('assets/data/quotes/en/$themeLower.json');
      return json.decode(jsonString) as Map<String, dynamic>;
    } catch (_) {
      // Ultimate fallback to English Happy theme
      final String jsonString = await rootBundle.loadString('assets/data/quotes/en/happy.json');
      return json.decode(jsonString) as Map<String, dynamic>;
    }
  }
});
...
class QuoteService {
  ...
  QuoteModel getQuote(DateTime time) {
    ...
    final String subtitleText = periodData?['subtitle']?.toString() ?? 'Inspiration for your journey';
    ...
    final int index = dayOfYear % quotesList.length;
...
final quoteServiceProvider = FutureProvider.family<QuoteService, String>((ref, theme) async {
  final themeData = await ref.watch(quoteDataProvider(theme).future);
  return QuoteService(themeData);
});
