    ...
    def transcribe(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            # Fallback to simulated text transcription if API call fails
            return {
                "text": "This is a simulated transcription. (FastAPI STT API Fallback)",
                "provider": "fallback",
                "model": "simulated",
                "language": language or "en",
                "error": str(e)
        ...
        **kwargs: Any,
        ...
        for chunk in chunks:
            ...
            try:
                ...
                response.raise_for_status()
                audio_content.extend(response.content)
            except Exception:
                # Return whatever we succeeded with, or raise if empty
                pass
                
        if not audio_content:
            # Fallback mock MP3 bytes (1-second silence file representation or simple dummy bytes)
