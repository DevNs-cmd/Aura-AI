...
let NotificationService = NotificationService_1 = class NotificationService {
    ...
    async sendPushNotification(fcmToken, title, body) {
        if (!fcmToken || !title || !body) {
            throw new common_1.BadRequestException('FCM token, title, and body are required.');
        }
        const payload = {
            token: fcmToken,
        ...
        try {
            ...
            throw new common_1.InternalServerErrorException(`FCM deliver failure: ${error.message}`);
        }
    }
};
